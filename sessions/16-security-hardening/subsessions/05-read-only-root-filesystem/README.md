# Sub-Session 05: Read Only Root Filesystem
...
```bash
kubectl apply -f subsessions/05-read-only-root-filesystem/
```
...
```bash
kubectl delete -f subsessions/05-read-only-root-filesystem/ --ignore-not-found
```
