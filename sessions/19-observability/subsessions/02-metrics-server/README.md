# Sub-Session 02: Metrics Server

This sub-session is part of 19-observability.

## Goal

resource metrics for `kubectl top` and autoscaling.
...
```bash
kubectl apply -f subsessions/02-metrics-server/
```
...
```bash
kubectl delete -f subsessions/02-metrics-server/ --ignore-not-found
```
