# Sub-Session 03: External Secrets Operator
...
```bash
kubectl apply -f subsessions/03-external-secrets-operator/
```
...
```bash
kubectl delete -f subsessions/03-external-secrets-operator/ --ignore-not-found
```
