# Sub-Session 06: Policy Examples

This sub-session is part of 25-admission-control-and-policy.

## Goal

require labels, restrict images, block privileged Pods.
...
```bash
kubectl apply -f subsessions/06-policy-examples/
```
...
```bash
kubectl delete -f subsessions/06-policy-examples/ --ignore-not-found
```
